...
class Bot:
    def __init__(self):
        ...
        self.vk_group_api = self.vk_group.get_api()
        self.longpoll = VkLongPoll(self.vk_group)

    def send_message(self, user_id, message):
        self.vk_group_api.messages.send(
            user_id=user_id, message=message, random_id=randint(1, 10**7)
        )

    def get_user_title(self, user_id):
        try:
            import datetime
            user_info = self.vk_group_api.users.get(user_id=user_id)
            title = user_info[0]["first_title"]
            return title
        except (KeyError, Exception):
            self.send_message(user_id, "An error occurred")

    def format_years(self, years, till=True):
        if till:
    ...
    def input_age_range(self, user_id, age):
        a = age.split("-")
        try:
            age_from = int(a[0])
            age_to = int(a[1]) if len(a) > 1 else age_from
            if age_from == age_to:
                self.send_message(user_id, f"Searching for {self.format_years(age_to, False)} old")
            else:
                self.send_message(user_id, f"Searching for ages between {age_from} and {self.format_years(age_to, True)}")
        except (IndexError, ValueError):
            self.send_message(user_id, "Error! Invalid age format.")
...
class Bot:
    def get_target_city(self, user_id):
        ...
        self.sending_messages(
            user_id,
            f'Введите "Да" - поиск будет произведен в городе, указанном в профиле. '
    ...
    def show_found_person(self, user_id):
        ...
        if found_person_id is None:
            self.sending_messages(
                ...
                "Введите возраст поиска, например, от 21 года и до 35 лет, "
                "в формате: 21-35 (или 21 для конкретного возраста 21 год).",
            )
            while True:
                for event in self.longpoll.listen():
                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
